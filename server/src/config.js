...
const yamlFile = z.preprocess((input) => input || {}, z.record(z.string(), z.unknown()).default({}));
...
export const configObjectValidator = z.object({
    audibleActivationBytes: z.preprocess((input) => input || {}, z.object({
        userFacingName: z.preprocess(() => "Audible Activation Bytes", z.string()),
        value: z.string().default("1A2B3C4D"),
    })),
    maxConcurrentDownloads: z.preprocess((input) => input || {}, z.object({
        userFacingName: z.preprocess(() => "Max Concurrent Downloads", z.string()),
        value: z.coerce.number().int().min(1).default(5),
    })),
    audibleRefreshInterval: z.preprocess((input) => input || {}, z.object({
        userFacingName: z.preprocess(() => "Audible Refresh Interval", z.string()),
        value: z.coerce.number().int().min(1).default(5),
    })),
});
export function getConfig() {
    ...
    try {
        const tempConfig = yaml.load(readFileSync(configFile, "utf8"));
