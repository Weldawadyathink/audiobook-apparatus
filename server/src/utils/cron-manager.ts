...
const globalForToad = globalThis as unknown as {
  scheduler: ToadScheduler | undefined;
  audibleTask: Task | undefined;
};
...
function getAudibleTask() {
  if (!globalForToad.audibleTask) {
    globalForToad.audibleTask = new Task("audibleJob", () =>
...
export function refreshAudibleJobSchedule() {
  ...
  scheduler.addSimpleIntervalJob(
    new SimpleIntervalJob(
      { minutes: newInterval, runImmediately: true },
      getAudibleTask(),
