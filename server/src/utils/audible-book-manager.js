import { convertAax, convertAaxc, downloadItem, getLibrary, } from "./audible-cli.ts";
...
async function reindexAudibleData() {
    ...
    const results = await db
        ...
        .from(book)
        .where(and(eq(book.source, "Audible"), isNotNull(book.asin), or(isNull(book.title), isNull(book.imageUrl), isNull(book.author), isNull(book.narrator))));
    ...
    async function updateBook(asin, retryNumber = 0) {
        const maxRetries = 5;
        await api
            .get(asin)
            .then(async (response) => {
            const data = audibleProductResponse.parse(response.product);
            await db
                .update(book)
                .set({
                title: data.title,
    ...
    await Promise.allSettled(results.map((item) => {
...
export async function downloadAudibleBook(asin) {
    ...
    await db
        ...
        .where(eq(book.asin, asin));
    const maxRetries = 5;
    async function doDownload(asin, progressFunction, retryNumber = 0) {
    ...
    if (downloadResult.voucherFilename) {
        ...
        await convertAaxc(downloadResult.filename, downloadResult.voucherFilename, outputFilepath);
    }
    else {
        console.log(`Converting ${asin} aax file`);
        await convertAax(downloadResult.filename, outputFilepath, getConfig().audibleActivationBytes.value);
    }
...
export async function downloadAll() {
    const booksToDownload = await db
        .select({ asin: book.asin })
        .from(book)
        .where(and(isNotNull(book.asin), eq(book.source, "Audible"), eq(book.status, "Not Downloaded"), isNotNull(book.title)));
